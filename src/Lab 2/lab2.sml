...
(* helper for middle *)
fun divvy(first,second) = if null(tl second) orelse null(tl(tl(second))) then hd first
  else divvy(tl first, tl(tl(second)));

fun middle([a]) = a
  | middle(lst) = divvy(lst,lst);

fun median(a,b,c) = if((a <= b andalso b <= c) orelse (c <= b andalso b <= a)) then b
  else if((b <= a andalso a <= c) orelse (c <= a andalso a <= b)) then a
  else c;

fun partition(nil, p) = (nil, nil)
  | partition([a],p) = if a <= p then ([a],nil) 
      else (nil,[a])
  | partition(a::bs,p) = let val (x,y) = partition(bs,p) in (if a <= p then (a::x,y)
      else (x,a::y)) end;

(* determines if all elements in the list are the same number *)
fun monotone(nil) = true
  | monotone([a]) = true
  | monotone(a::b::nil) = if a = b then true
      else false
  | monotone(a::b::cs) = if a = b then (true andalso monotone(cs))
      else false;
